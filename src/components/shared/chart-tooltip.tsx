...
export function ChartTooltipContent({
  ...
  return (
    <div className="rounded-xl border border-border bg-card-elevated px-3 py-2 shadow-card">
      {label ? <p className="mb-1 text-ui-xs text-muted-foreground">{label}</p> : null}
      <div className="space-y-0.5">
        {payload.map((item) => (
          <p key={String(item.dataKey ?? item.name)} className="flex items-center gap-2 text-xs">
            ...
            <span className="money font-medium text-foreground">{formatMoney(Number(item.value), currency)}</span>
          </p>
        ))}
      </div>
    </div>
