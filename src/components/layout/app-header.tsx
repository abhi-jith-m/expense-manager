  ...
  return (
    <header className="sticky top-0 z-[var(--z-nav)] flex h-[calc(var(--header-height)+env(safe-area-inset-top))] shrink-0 items-center gap-2 border-b border-border bg-background/90 px-[var(--page-pad)] pt-[env(safe-area-inset-top)] backdrop-blur-md">
      <button
        ...
        className="flex size-11 shrink-0 items-center justify-center rounded-[12px] text-foreground lg:hidden"
      >
        <Menu className="size-5" />
      </button>
      <p className="min-w-0 flex-1 truncate text-base font-semibold tracking-tight">{pageTitle(location.pathname)}</p>
      <div className="flex shrink-0 items-center gap-1">
        <VioTrigger />
        <Link
          to="/notifications"
          aria-label="Notifications"
          className="relative flex size-11 items-center justify-center rounded-[12px] text-muted-foreground"
        >
