...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader><CardTitle>Appearance</CardTitle><CardDescription>Theme is persisted on this device and in your profile.</CardDescription></CardHeader>
        <CardContent>
          <Select value={user.theme} onValueChange={(value) => {
      ...
      <Card>
        <CardHeader><CardTitle>Currency and dates</CardTitle><CardDescription>Amounts are never auto-converted between currencies.</CardDescription></CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2">
          <div className="space-y-1.5">
            ...
            <Select value={user.currency} onValueChange={(value) => void patch({ currency: value })}>
              ...
              <SelectContent>
                {CURRENCIES.map((item) => <SelectItem key={item.code} value={item.code}>{item.code} · {item.name}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1.5">
            ...
            <Select value={user.dateFormat} onValueChange={(value) => void patch({ dateFormat: value })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {DATE_FORMATS.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader><CardTitle>Data</CardTitle><CardDescription>Sample data is tagged and can be removed without touching real records.</CardDescription></CardHeader>
        <CardContent className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={() => navigate('/import-export')}>Import data</Button>
          <Button variant="outline" onClick={async () => {
            ...
            for (const budget of seed.budgets(categories.data ?? [])) await client.createBudget({
              name: budget.name, categoryId: budget.categoryId, limitAmount: budget.limitAmount, period: budget.period, startDate: budget.startDate, endDate: budget.endDate, alertThreshold: budget.alertThreshold,
            })
            for (const goal of seed.goals) await client.createGoal({
              name: goal.name, targetAmount: goal.targetAmount, currentAmount: goal.currentAmount, deadline: goal.deadline, icon: goal.icon, color: goal.color,
            })
            invalidate()
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-wrap gap-2">
            <Input type="password" placeholder="New password" value={password} onChange={(event) => setPassword(event.target.value)} className="max-w-xs" />
            <Button onClick={() => void client.updatePassword(password).then(() => { toast.success('Password updated'); setPassword('') }).catch((error) => toast.error(toUserMessage(error)))}>Change password</Button>
          </div>
          <Button variant="outline" onClick={() => void signOut()}>Log out of this session</Button>
